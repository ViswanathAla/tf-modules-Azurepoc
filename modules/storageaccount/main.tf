...
resource "azurerm_storage_account" "storeacc" {
  ...
  is_hns_enabled           = var.enable_adls
...
resource "azurerm_storage_data_lake_gen2_filesystem" "adls-sto" {
   count             = var.enable_adls == true ? 1 : 0
  name               = "${var.storage_account_name}-adls"
  storage_account_id = azurerm_storage_account.storeacc.id
